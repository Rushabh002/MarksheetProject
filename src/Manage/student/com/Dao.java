...
public class Dao {
    ...
    public static void updateStudentName(){
        ...
        String name = validateName(sc);
        Students students =new Students(roll,name);

        try {
            String query ="UPDATE students set name = ? where id =?";
            PreparedStatement preparedStatement = con.prepareStatement(query);
            preparedStatement.setString(1,students.setStud_name(name));
    ...
    public static void updateAge(){
        ...
        int age = validateAge(sc);
        Students students = new Students(roll,age);

        try {
            String query = "UPDATE students set age =? WHERE id = ?";
            PreparedStatement preparedStatement= con.prepareStatement(query);
            preparedStatement.setInt(1,students.setStud_age(age));
            preparedStatement.setInt(2,students.getId());
            int row = preparedStatement.executeUpdate();
            if (row > 0) {
    ...
    public static void deleteStudentRecord(){
        ...
        int roll = sc.nextInt();
        Students students = new Students(roll);
    ...
    public static void updateAllmarks(){
        ...
        try {
            ...
            PreparedStatement preparedStatement= con.prepareStatement(query);
            preparedStatement.setInt(1,marksheets.setPhysics(physics));
            preparedStatement.setInt(2,marksheets.setChemistry(chemistry));
            preparedStatement.setInt(3,marksheets.setMaths(maths));
            preparedStatement.setInt(4,marksheets.setEnglish(eng));
    ...
    private static String validateName(Scanner sc) {
        ...
        do {
            ...
            if (name.isEmpty() || name.length() > 20) {
                System.out.println("Invalid name. Please enter a valid name (up to 50 characters).");
            }
